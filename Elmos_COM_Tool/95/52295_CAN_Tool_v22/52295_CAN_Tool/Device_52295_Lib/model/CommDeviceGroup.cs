...
namespace Device_52295_Lib
{
    ...
    public class CommDeviceGroup
    {
        ...
        public void WriteCurrentsData()
        {
            byte[] zeroCurrents = new byte[0];

            byte[] currents_0 = zeroCurrents;
            byte[] currents_1 = zeroCurrents;
            byte[] currents_2 = zeroCurrents;

            foreach (CommDevice d in _commDevicesRef)
            {
                Debug.Assert(d.canGroup == _group);

                if (d.canIndex == 0) currents_0 = new byte[BusConfig.SIZE_CURRENT_AREA];
                if (d.canIndex == 1) currents_1 = new byte[BusConfig.SIZE_CURRENT_AREA];
                if (d.canIndex == 2) currents_2 = new byte[BusConfig.SIZE_CURRENT_AREA];

                for (byte i = 0; i < BusConfig.SIZE_CURRENT_AREA; i += 1)
                {
                    if (d.canIndex == 0) currents_0[i] = d.deviceRef.busConfig.GetCurrent(i);
                    if (d.canIndex == 1) currents_1[i] = d.deviceRef.busConfig.GetCurrent(i);
                    if (d.canIndex == 2) currents_2[i] = d.deviceRef.busConfig.GetCurrent(i);
                }
            }

            _WriteW3Group(BusConfig.ADDR_CURRENT_AREA, currents_0, BusConfig.ADDR_CURRENT_AREA, currents_1, BusConfig.ADDR_CURRENT_AREA, currents_2);
        }
    }
